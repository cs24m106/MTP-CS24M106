...
% ── ViMo-Flow results are in Appendix~\ref{sec:appendix_vimo}. ────────────────
% ── Physics-Based Imitation results will be added here once           ─────────
% ── frame-wise simulation renders and training curve plots are ready. ─────────
...
These results demonstrate that the body-part discriminator approach scales effectively to simple static and cyclic motions (squat, punch) but exposes limits on high-momentum asymmetric or ground-interaction behaviors (kick, roll). Overlapping frame composites and per-episode RGB arrays (generated via \texttt{eval\_frames.py}) provide visual confirmation, with training curves (via \texttt{eval\_analyzer.py}) showing rapid convergence for high-fidelity cases within 5\,000--10\,000 epochs.
