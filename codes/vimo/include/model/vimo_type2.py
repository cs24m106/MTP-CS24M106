...
class PositionalEncoding(nn.Module):
    ...
    def sequential_forward(self, x): # Forward for sequence data: x shape (B,S,C)
        assert x.size(-1) == self.pe.size(-1), f"PE_seq: Feature Dimension (d_model:{self.d_model} != input_last_dim:{x.size(-1)}) mismatch!"
        S = x.size(1)
    ...
    def forward(self, x):
        ...
        if x.size(-1) == 1: # timestep data (B,1) or (B,S,1)
            B = x.size(0); S = x.size(1)
            if S == 1: return self.timestep_forward(x.squeeze(1))
            pe_all = self.timestep_forward(x.reshape(B*S)) # make it 1D list for indexing
            return pe_all.reshape(B, S, self.d_model)
        else:  # sequence data
            return self.sequential_forward(x)


# ======================== ViMo-Denoiser FrameWork Version 1 ========================

class DenoiserBlockv1(nn.Module):
...
class ViMoFrameWorkv1(nn.Module):
    """
    Parameters:
    ...
    def __init__(self, motion_dim=151, pose_dim=17*3, max_T=1000, embed_dim=256, n_heads=8, m_nlayers=3, p_nlayers=2, cond_drop=0.25, verbose=False):
        ...
        self.d_modules = nn.ModuleList([DenoiserBlockv1(embed_dim, n_heads) for _ in range(m_nlayers)])
    ...
    def forward(self, x_t, timestep, p2d=None, drop_off=None):
        ...
        if timestep.dim() == 1 or timestep.size(-1) != 1:
            timestep = timestep.unsqueeze(-1)  # (B,) or (B,S) -> (B,1) or (B,S,1) our common PE expects in this form
        ...
        # timestep embedding
        t_pos = self.PE(timestep)  # sinusoidal positional embeddings (B,1)||(B,S,1) -> (B,1, embed_dim=d)||(B,S,embed_dim=d)
        
        if self.verbose: print(f"t_pos.shape: {t_pos.shape},", end=" ")
        t_emb = self.timestep_embedding(t_pos)  # (B,d) || (B,S,d) ----- ----- ----- ----- ----- ----- ----- (1)
        if self.verbose: print(f"t_emb.shape: {t_emb.shape} ...")
        
        if timestep.dim() == 2: # no PTSS
            pt_emb = p_encoded + t_pos.unsqueeze(1) # (B,S,d) + (B,1,d) => (B,S,d)
        else: # with PTSS
            pt_emb = p_encoded + t_pos # (B,S,d) Add time position encodings to pose tokens
        if self.verbose: print(f"pt_emb.shape: {pt_emb.shape},", end=" ") # (B,S,d)
        
        # Mean pooling (no PTSS) => (B, sum(embed_dim)/S_frames) i.e. mean over time
        p_avg = pt_emb.mean(dim=1) if timestep.dim() == 2 else pt_emb # (B,d) || (B,S,d) ----- ----- ------- (2)
        if self.verbose: print(f"p_avg.shape: {p_avg.shape},", end=" ")

        # combine encoded(pose), embedding(timestep) -> FiLM params
        film_in = torch.cat([p_avg, t_emb], dim=-1) # concat on last dim => (B,d) || (B,S,d) ----- -----  (1,2)
        if self.verbose: print(f"film_in.shape: {film_in.shape},", end=" ")
        film_params = self.film_mlp(film_in)  # (B, 2*embed_dim) || (B, S, 2*embed_dim)
        ...
        if film_in.dim() == 2: # no PTSS => expand (B,1,d) to (B,S,d)
            gamma = gamma.unsqueeze(1).expand(-1, S, -1); beta  = beta.unsqueeze(1).expand(-1, S, -1)
    ...
    @property # compact name for files
    def compact_name(self):
        return f"ViMo(m={self.motion_dim},p={self.pose_dim},t={self.max_T},d={self.embed_dim},h={self.n_heads}," + \
                f"ml={self.m_nlayers},pl={self.p_nlayers},c={self.cond_drop})"


# ======================== ViMo-Denoiser FrameWork Version 2 ========================
...
class DenoiserBlockv2(nn.Module):
    def __init__(self, d_model, n_heads):
        ...
        self.norm_ca = nn.LayerNorm(d_model) # layer norm after cross-attn
        self.norm_ff = nn.LayerNorm(d_model) # layer norm after ff-mlp
        # seperate layers to produce FiLM parameters from concatenated[encoded(pose); embeded(timestep)]
        film_dim = d_model*2
        self.film_sa = nn.Linear(film_dim, film_dim) # out = (B, 2*embed_dim)
        self.film_ca = nn.Linear(film_dim, film_dim)
        self.film_ff = nn.Linear(film_dim, film_dim)
        for layer in [self.film_sa, self.film_ca, self.film_ff]:
            nn.init.zeros_(layer.weight); nn.init.zeros_(layer.bias) # to preserve identify intial training

    def forward(self, motion_tokens, pose_tokens, film_inp): # film params remains same within single block?
        # because gamma & beta are generated from conditions => [encoded(pose), embed(timestep)] (which are not supposed to be constant)
        x = motion_tokens # alias overwriting after each block
        B, S, _ = x.shape
        ...
        sa = self.self_attn(query=x, key=x, value=x, need_weights=False)[0]
        x = self.norm_sa(x + sa)
        gamma, beta = self.film_sa(film_inp).chunk(2, dim=-1) # split 2 chunks across last dim
        if film_inp.dim() == 2: # no PTSS => expand (B,1,d) to (B,S,d)
            gamma = gamma.unsqueeze(1).expand(-1, S, -1); beta  = beta.unsqueeze(1).expand(-1, S, -1)
        x = x * (1 + gamma) + beta # apply FiLM after self-attn
        ...
        ca = self.cross_attn(query=x, key=pose_tokens, value=pose_tokens, need_weights=False)[0]
        x = self.norm_ca(x + ca)
        gamma, beta = self.film_ca(film_inp).chunk(2, dim=-1) # expands/broadcasts automatically (B,1,d) to (B,S,d)
        if film_inp.dim() == 2: # no PTSS => expand (B,1,d) to (B,S,d)
            gamma = gamma.unsqueeze(1).expand(-1, S, -1); beta  = beta.unsqueeze(1).expand(-1, S, -1)
        x = x * (1 + gamma) + beta # apply FiLM after self-attn
        ...
        fm = self.ff_mlp(x)
        x = self.norm_ff(x + fm)
        gamma, beta = self.film_ff(film_inp).chunk(2, dim=-1) # seperate FiLm params to avoid exponential scaling
        if film_inp.dim() == 2: # no PTSS => expand (B,1,d) to (B,S,d)
            gamma = gamma.unsqueeze(1).expand(-1, S, -1); beta  = beta.unsqueeze(1).expand(-1, S, -1)
        x = x * (1 + gamma) + beta # apply FiLM after MLP
        return x
...
class ViMoFrameWorkv2(nn.Module):
    """
    Parameters:
        - motion_dim: flattened dimension of 3D motion input (default 24*6+4+3=151)
        - pose_dim: flattened dimension of 2D pose input (default 17*3=51)
    ...
    def __init__(self, motion_dim=151, pose_dim=17*3, max_T=1000, embed_dim=256, n_heads=8, m_nlayers=3, p_nlayers=2, cond_drop=0.25, verbose=False):
        ...
        # stack of denoising modules (each contains self-attn, cross-attn, MLP)
        self.d_modules = nn.ModuleList([DenoiserBlockv2(embed_dim, n_heads) for _ in range(m_nlayers)])
    ...
    def forward(self, x_t, timestep, p2d=None, drop_off=None):
        ...
        if self.verbose: print(f"m3d_PE.shape: {x.shape},", end=" ")
        for denoiser in self.d_modules:
            x = denoiser(x, pt_emb, film_in)  # (S,B,d)
            if self.verbose: print(f"m3d_denoiser.shape: {x.shape},", end=" ")
    ...
    @property # adding device property for easier access
    def device(self):
        return next(self.parameters()).device
