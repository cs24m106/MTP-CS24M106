...
# ======================== ViMo-Denoiser FrameWork Version 1 (Common FiLM block params) ========================

class LightDenoiserBlock(nn.Module):
...
class LightViMoFramework(nn.Module):
	"""
	Parameters:
	...
	def __init__(self, motion_dim=151, pose_dim=17*3, max_T=1000, embed_dim=256, n_heads=8, m_nlayers=3, p_nlayers=2, cond_drop=0.25, verbose=False):
		...
		self.d_modules = nn.ModuleList([LightDenoiserBlock(embed_dim, n_heads) for _ in range(m_nlayers)])
...
# ======================== ViMo-Denoiser FrameWork Version 2 (Seperate FiLM block params) ========================

class HeavyDenoiserBlock(nn.Module):
    def __init__(self, d_model, n_heads):
        super().__init__()
        # fundamental blocks of denoiser
        self.self_attn = nn.MultiheadAttention(embed_dim=d_model, num_heads=n_heads, batch_first=True) # self-attention for motion tokens
    ...
    def forward(self, motion_tokens, pose_tokens, film_inp): # film params remains same within single block?
        ...
        sa = self.self_attn(query=x, key=x, value=x, need_weights=False)[0]
        x = self.norm_sa(x + sa)
        gamma, beta = self.film_sa(film_inp).chunk(2, dim=-1) # split 2 chunks across last dim, so each (B, embed_dim)
        x = x * (1 + gamma.unsqueeze(1)) + beta.unsqueeze(1) # apply FiLM after self-attn
        ...
        x = x * (1 + gamma.unsqueeze(1)) + beta.unsqueeze(1) # apply FiLM after MLP
        return x


class HeavyViMoFramework(nn.Module):
    """
    Parameters:
        - motion_dim: flattened dimension of 3D motion input (default 24*6+4+3=151)
        - pose_dim: flattened dimension of 2D pose input (default 17*3=51)
    ...
    def __init__(self, motion_dim=151, pose_dim=17*3, max_T=1000, embed_dim=256, n_heads=8, m_nlayers=3, p_nlayers=2, cond_drop=0.25, verbose=False):
        ...
        # stack of denoising modules (each contains self-attn, cross-attn, MLP)
        self.d_modules = nn.ModuleList([HeavyDenoiserBlock(embed_dim, n_heads) for _ in range(m_nlayers)])
