...
for label, rep in all_reports.items():
    ...
    if pct_ep < 10:
        print("  1. Check contact termination threshold (lower from 0.15m if too aggressive).")
        print("  2. Verify kp values don't create oscillation — inspect joint tracking in viewer.")
    if disc_r_slope > 0 and pct_clip < 50:
        print("  1. Continue training — disc_reward improving. Need 5k–20k epochs for meaningful motion.")
        print("  2. Consider increasing num_envs or horizon to generate more diverse rollouts.")
    if rep["lt_slope"] < 0:
        print("  - Lifetime declining: run is destabilising. Check if terminate_reward is sufficient.")
    print("  - See parameter recommendations section below for speed-up strategies.")
    print()
    '''
# ── PARAMETER RECOMMENDATIONS ─────────────────────────────────────────────────
'''
sep()
print("  PARAMETER & ARCHITECTURE RECOMMENDATIONS")
sep()
print("""
━━ XML PHYSICS TUNING ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Current kp values were set equal to original stiffness. This is conservative.
Recommended direction:

  Joint group   | Current kp | Suggested kp | Reason
  ──────────────────────────────────────────────────────────────────────
  Hip/knee      | 300        | 400–500      | Legs need authority to resist gravity
  Ankle         | 200        | 300–400      | Critical for balance; often underpowered
  Abdomen       | 600        | 400–500      | Overdamped — reduces torso flexibility
  Shoulder/arm  | 150–200    | 100–150      | Arms don't carry weight; lower = less noise
  Neck          | 50         | 30–50        | Fine

  kv (velocity damping) if using MuJoCo <position kv="...">:
    Set kv ≈ 2*sqrt(kp * inertia). Rough rule: kv = 0.1 * kp.
    e.g. hip kp=400 → kv=40. Ankle kp=300 → kv=30.

━━ TERMINATION THRESHOLD ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Current 0.15m is triggering on normal knee-bending. Suggested fixes:

  Option A (easiest): Lower height threshold 0.15m → 0.08m.
    This only catches true ground contact (shin/torso on floor).
    Normal walking has shins ~0.1m above ground.

  Option B (more forgiving): Add a "grace counter" — require N=3 consecutive
    frames below threshold before terminating. Prevents single-step spikes.

  Option C (recommended for learning speed): Combine A + B.
    threshold=0.08m, grace_steps=3.

━━ TRAINING SPEED: HYPERPARAMETERS ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  Param          | Current | Suggested | Effect
  ───────────────────────────────────────────────────────────────────
  horizon        | 8       | 16–32     | Longer rollouts = better GAE advantage estimates;
                                         also means the disc sees more context per update.
  batch_size     | 256     | 512–1024  | Larger batches = more stable gradient, esp. for disc.
                                         Increase only if horizon also increased.
  opt_epochs     | 5       | 3–4       | Fewer re-uses of stale data = less overfit on bad rollouts.
  actor_lr       | 5e-6    | 5e-6      | Already conservative — keep.
  critic_lr      | 1e-4    | 5e-5      | Slightly lower prevents value overfit to current bad policy.
  disc_lr        | 1e-5    | 5e-6      | Slower disc update = fairer competition with policy.
                                         If gap keeps growing, reducing disc_lr is the right lever.
  gamma          | 0.95    | 0.95      | Fine for 10-step episodes. Would need 0.99 for 300-step.
  lambda_        | 0.95    | 0.95      | Fine — keep.
  entropy_coeff  | (none)  | 0.005     | Add entropy bonus to actor loss. Prevents premature
                                         convergence to "collapse and stay still" policy.

━━ TRAINING SPEED: SCHEDULE ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  - Linear warmup + cosine decay for actor_lr: warm up for 500 epochs,
    then decay to 1e-7 by epoch 20k. Prevents large early updates from
    locking in a bad policy before the critic converges.
  - Early in training (epoch <1000), the disc dominates. Temporarily
    reducing disc_lr to 2e-6 for first 1000 epochs levels the field.

━━ ARCHITECTURE IMPROVEMENTS ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  (1) PHASE INPUT — most important for cyclical motions like walking.
      Add a "motion phase" scalar to the policy observation (0→1 over clip).
      Without phase, the network has to infer timing from kinematics alone.
      Paper: AMP (Peng et al. 2021), Section 3 — phase-conditioned policies
      converge 3–5x faster on periodic motions.

  (2) OBSERVATION NORMALISATION — running mean/std normalisation on the
      input observation vector (separate from action normalisation).
      Important because different DOFs have different typical magnitudes.
      Implementation: maintain exponential running stats, normalise before
      actor + critic forward pass. Ref: PPO paper (Schulman et al., 2017).

  (3) DISCRIMINATOR GRU SIZE — current ob_horizon=5 is small.
      Consider ob_horizon=8–10 for walking (one stride ~ 0.7s = 21 frames).
      Also consider adding a 2-layer GRU instead of 1-layer for richer
      temporal features. Ref: ICCGAN paper Section 4.2.

  (4) REWARD SCALING — scale disc_reward to keep advantages in [−3, +3].
      If disc_reward oscillates between −0.15 and +0.01, multiply by 5
      to widen the signal. This helps PPO assign meaningful advantage values
      when reward variance is low.

  (5) SYMMETRY LOSS — add a bilateral symmetry regularisation term to the
      policy: L_sym = ||π(s) - flip(π(flip(s)))||². Particularly effective
      for bipedal locomotion; improves generalisation and reduces falls.
      Ref: DeepMimic (Peng et al. 2018), symmetric reward variant.
""")
'''
